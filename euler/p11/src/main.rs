...
fn main() {
    ...
    println!("max product: {}", products.into_iter().max().unwrap());
}

fn get_horizontal_max(graph: Vec<Vec<u64>>) -> u64 {
    let mut products: Vec<u64> = Vec::new();
    for line in graph {
        for n in line.chunks(4){
            products.push(n.to_vec().iter().product()); 
        }
    }
    return products.iter().max().unwrap().clone();
}

fn get_vertical_max(graph: Vec<Vec<u64>>) -> u64 {
    let mut newgraph: Vec<Vec<u64>> = Vec::new();
    ...
    return get_horizontal_max(newgraph.clone());
}

fn get_diagonal_max(graph: Vec<Vec<u64>>) -> u64 {
    //just shift stuff over by adding 0s in increasing amounts
    //000xxx0
    //00xxx00 
    //0xxx000
    //xxx0000  max num of zeroes is len - 1
    //add based on max - index
    let maxshift = graph.len() - 1;
    let mut leftshift: Vec<Vec<u64>> = graph.clone();
    let mut rightshift: Vec<Vec<u64>> = graph.clone();
    //shift left and right
    for zeroes in (1..maxshift + 1).rev() {
        for _ in 0..zeroes {
            leftshift[maxshift - zeroes].insert(0,0);
            rightshift[maxshift - zeroes].push(0);
        }
        for _ in 0..zeroes {
            leftshift[zeroes].push(0); 
            rightshift[zeroes].insert(0,0);
        }
    }
    return vec![get_vertical_max(leftshift).clone(),get_vertical_max(rightshift).clone()]
        .into_iter()
        .max()
        .unwrap();
}
